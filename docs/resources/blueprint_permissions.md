...
- `read_properties` (Attributes Map) Manage permissions to read specific entity properties. Meta properties starting with `$` must be configured under `read_metadata_properties`. The `$identifier` meta property and mirror properties cannot have read restrictions. (see [below for nested schema](#nestedatt--entities--read_properties))
