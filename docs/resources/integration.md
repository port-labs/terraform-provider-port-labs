...
- `oauth_broker_url` (String) The base URL of the OAuth broker hosted by this integration. Port uses this URL to build the re-authentication link sent to users when the integration requires a new token. Must not contain a query string.
- `title` (String)
- `version` (String)
