...
- `properties` (Map of String) Additional `_scorecard` blueprint properties applied to every member scorecard in the group. Keys must match properties defined on the scorecard blueprint schema. Values that are not plain strings should be encoded with `jsonencode()`.
