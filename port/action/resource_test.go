...
func TestAccPortActionWebhookInvocation(t *testing.T) {
	...
	resource "port_action" "create_microservice" {
		...
		webhook_method = {
			...
			description = "Invoke the restart webhook"
			timeout_seconds = 30
		}
	}`, actionIdentifier)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("port_action.create_microservice", "webhook_method.description", "Invoke the restart webhook"),
					resource.TestCheckResourceAttr("port_action.create_microservice", "webhook_method.timeout_seconds", "30"),
				),
			},
		},
	})
...
func TestAccPortActionSteps(t *testing.T) {
	identifier := utils.GenID()
	actionIdentifier := utils.GenID()
	var testAccActionConfigCreate = testAccCreateBlueprintConfig(identifier) + fmt.Sprintf(`
	resource "port_action" "create_microservice" {
...
func TestAccPortActionStepsVisibleJqQuery(t *testing.T) {
	identifier := utils.GenID()
	actionIdentifier := utils.GenID()
	var testAccActionConfigCreate = testAccCreateBlueprintConfig(identifier) + fmt.Sprintf(`
	resource "port_action" "create_microservice" {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("port_action.create_microservice", "self_service_trigger.steps.1.title", "Step #2"),
					resource.TestCheckResourceAttr("port_action.create_microservice", "self_service_trigger.steps.1.order.0", "prop2"),
					resource.TestCheckResourceAttr("port_action.create_microservice", "self_service_trigger.steps.1.visible_jq_query", "1==1"),
				),
			},
		},
	})
