...
func TestAccPortActionWebhookInvocation(t *testing.T) {
	...
	resource "port_action" "create_microservice" {
		...
		webhook_method = {
			...
			label = "Example webhook invocation"
			timeout_seconds = 60
		}
	}`, actionIdentifier)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("port_action.create_microservice", "webhook_method.label", "Example webhook invocation"),
					resource.TestCheckResourceAttr("port_action.create_microservice", "webhook_method.timeout_seconds", "60"),
				),
			},
		},
	})
