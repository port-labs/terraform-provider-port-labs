...
func refreshBlueprintPermissionsState(state *BlueprintPermissionsModel, a *cli.BlueprintPermissions, blueprintId string, jsonEscapeHTML bool, mirrorPropertyKeys map[string]struct{}) error {
	...
	if len(a.Entities.ReadProperties) > 0 {
		...
		for readPropertyKey, readPropertyValue := range a.Entities.ReadProperties {
			...
			if _, isMirrorProperty := mirrorPropertyKeys[readPropertyKey]; isMirrorProperty {
				continue
			}
