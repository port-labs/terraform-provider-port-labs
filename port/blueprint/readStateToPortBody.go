...
// Helper functions to merge state with existing blueprint
func MergeProperties(ctx context.Context, existing map[string]cli.BlueprintProperty, state *PropertiesModel) (map[string]cli.BlueprintProperty, []string, error) {
	...
	// Add or update properties from state
	if state != nil {
		props, required, err := PropertiesToBody(ctx, state)
		if err != nil {
			return nil, nil, err
		}
		for k, v := range props {
...
func MergeRelations(existing map[string]cli.Relation, state map[string]RelationModel) map[string]cli.Relation {
	// Start with the existing relations
	merged := make(map[string]cli.Relation)
	for k, v := range existing {
		merged[k] = v
	}

	// Add or update relations from state
	if state != nil {
		relations := RelationsToBody(state)
...
func MergeMirrorProperties(existing map[string]cli.BlueprintMirrorProperty, state map[string]MirrorPropertyModel) map[string]cli.BlueprintMirrorProperty {
	// Start with the existing mirror properties
	merged := make(map[string]cli.BlueprintMirrorProperty)
	for k, v := range existing {
		merged[k] = v
	}

	// Add or update mirror properties from state
	if state != nil {
		mirrorProps := MirrorPropertiesToBody(state)
...
func MergeCalculationProperties(ctx context.Context, existing map[string]cli.BlueprintCalculationProperty, state map[string]CalculationPropertyModel) map[string]cli.BlueprintCalculationProperty {
	// Start with the existing calculation properties
	merged := make(map[string]cli.BlueprintCalculationProperty)
	for k, v := range existing {
		merged[k] = v
	}

	// Add or update calculation properties from state
	if state != nil {
		calcProps := CalculationPropertiesToBody(ctx, state)
