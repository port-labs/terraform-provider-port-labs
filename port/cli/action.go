...
func (c *PortClient) ReadAction(ctx context.Context, blueprintID, id string) (*Action, error, *PortError) {
	pb := &PortBody{}
	pe := &PortError{}
	...
	if !pb.OK {
		return nil, fmt.Errorf("failed to read action, got: %s", resp.Body()), pe
	}
	return &pb.Action, nil, pe
}
