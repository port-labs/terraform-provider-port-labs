...
func supportedFeaturesToPortBody(features *SupportedFeaturesModel) map[string]bool {
	...
	setBoolIfKnown(result, "useLegacyMaxTokens", features.UseLegacyMaxTokens)
...
func supportedFeaturesFromPort(features map[string]any) *SupportedFeaturesModel {
	...
	setFrameworkBoolIfPresent(result, &result.UseLegacyMaxTokens, features, "useLegacyMaxTokens")
	...
	if result.Temperature.IsNull() &&
		...
		result.UseLegacyMaxTokens.IsNull() {
		return nil
	}
