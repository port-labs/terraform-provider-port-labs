...
func TestOpenAICompatibleUseLegacyMaxTokens(t *testing.T) {
	state := &llm_provider.LLMProviderModel{
		ProviderType: types.StringValue("openai-compatible"),
		Enabled:      types.BoolValue(true),
		OpenAICompatible: &llm_provider.OpenAICompatibleModel{
			BaseUrl:          types.StringValue("https://api.example.com/openai"),
			ApiKeySecretName: types.StringValue("MY_KEY"),
			Models: []llm_provider.CustomModelModel{
				{
					Name: types.StringValue("gpt-4o"),
					SupportedFeatures: &llm_provider.SupportedFeaturesModel{
						UseLegacyMaxTokens: types.BoolValue(false),
					},
				},
			},
		},
	...
	body, err := llm_provider.LlmProviderToPortBodyForTest(state)
	require.NoError(t, err)
	assert.Equal(t, "openai-compatible", body.Provider)
	...
	features, ok := models[0]["supportedFeatures"].(map[string]bool)
	require.True(t, ok)
	assert.False(t, features["useLegacyMaxTokens"])
}
