...
func TestScorecardGroupResourceToPortBodyProperties(t *testing.T) {
	...
	state := &ScorecardGroupModel{
		Identifier: types.StringValue("group-3"),
		Title:      types.StringValue("Group 3"),
	...
	if group.Properties["test_string"] != "shared-value" {
		t.Fatalf("unexpected test_string property: %v", group.Properties["test_string"])
	}
	if group.Properties["test"] != "https://example.com" {
		t.Fatalf("unexpected test property: %v", group.Properties["test"])
	}
}
